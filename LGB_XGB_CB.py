...
def train_stage(df_path, lgb_path, xgb_path, cb_path):
    ...
    auc_mean_lgb_xgb = round(roc_auc_score(y_df, (lgb_cv_result + xgb_cv_result) / 2), 4)
    ...
    print('Mean XGBoost+LightGBM VAL AUC: {}'.format(auc_mean_lgb_xgb))
    print('Mean Catboost+XGBoost VAL AUC: {}'.format(auc_mean_xgb_cb))
...
def prediction_stage(df_path, lgb_path, xgb_path, cb_path):
    ...
    submission = pd.read_csv('../SantanderData/sample_submission.csv')
    submission['target'] = (lgb_result + xgb_result + cb_result) / 3
    submission.to_csv('../SantanderData/xgb_lgb_cb_starter_submission.csv', index=False)
    submission['target'] = (lgb_result + cb_result) / 2
    submission.to_csv('../SantanderData/lgb_cb_starter_submission.csv', index=False)
    submission['target'] = (lgb_result + xgb_result) / 2
    submission.to_csv('../SantanderData/lgb_xgb_starter_submission.csv', index=False)
    submission['target'] = (xgb_result + cb_result) / 2
    submission.to_csv('../SantanderData/xgb_cb_starter_submission.csv', index=False)
    submission['target'] = xgb_result
    submission.to_csv('../SantanderData/xgb_starter_submission.csv', index=False)
    submission['target'] = lgb_result
    submission.to_csv('../SantanderData/lgb_starter_submission.csv', index=False)
    submission['target'] = cb_result
    submission.to_csv('../SantanderData/cb_starter_submission.csv', index=False)

    return 0


if __name__ == '__main__':
    train_path = '../SantanderData/train.csv'
    test_path = '../SantanderData/test.csv'
